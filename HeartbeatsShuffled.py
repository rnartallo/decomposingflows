...
ppt = np.array(pd.read_csv('ppt.csv',header=None),dtype = int)#random.sample(range(0,10506),4046)
...
for counter in range(0,2):
    ...
    for s in range(0,bootstraps):
        ...
        Shuffle_Curl.append(pipeline(shuffled_phase))
    np.savetxt(names[counter]+"ShuffOrig.csv", np.array(Curl), delimiter=",")
    np.savetxt(names[counter]+"ShuffShuff.csv", np.array(Shuffle_Curl), delimiter=",")

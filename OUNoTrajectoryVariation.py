...
traj_range = np.linspace(50,1000,20)
Curl =[]

for e in range(0,20):
    print(e)
    no_trajectories = traj_range[e]
    [C,G] = pipeline(int(no_trajectories))
    Curl.append(C)


np.savetxt("CurlIsoOU2NoTraj.csv", np.array(Curl), delimiter=",")
